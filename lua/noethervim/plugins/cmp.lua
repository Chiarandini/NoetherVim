			...
			sources = {
				...
				providers = {
					...
					snippets = {
						...
						-- Hide snippets whose trigger is a pattern rather than
						-- literal text. blink's adapter sets `insertText =
						-- snip.trigger` on every item, so accepting one of these
						-- pastes the pattern itself: `:defn` would yield
